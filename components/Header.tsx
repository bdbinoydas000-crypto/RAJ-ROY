            ...
            <div className="container mx-auto px-4 py-3 flex justify-between items-center gap-4">
                ...
                <div className="relative flex-grow max-w-xl mx-4">
                    <input 
                        ...
                        className="w-full bg-gray-800/50 border border-gray-700 rounded-full px-5 py-2 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
                    />
                     <div className="absolute top-0 right-0 h-full flex items-center pr-4 text-gray-500">
                ...
                <nav className="flex items-center space-x-2 md:space-x-4 flex-shrink-0">
                    ...
                    {isAuthenticated ? (
                         <>
                            <button onClick={() => navigateTo('profile')} className="hover:text-purple-400 transition-colors" title={t('myProfile')}>
                             ...
                             <div className="flex items-center space-x-1 text-yellow-400" title={`${rewardPoints} ${t('rewardPoints')}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        <button onClick={onAuthClick} className="text-sm font-medium hover:text-purple-400 transition-colors">
                            {t('loginSignUp')}
                        </button>
                    )}
                    ...
                    <button onClick={() => setCartOpen(true)} className="relative hover:text-purple-400 transition-colors">
                     ...
                     {isAuthenticated && (
                         <button onClick={onLogout} className="text-sm font-medium hover:text-pink-500 transition-colors hidden md:block">
                            {t('logout')}
                        </button>
                    )}
                </nav>
